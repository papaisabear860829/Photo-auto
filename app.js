...
async function loadModel() {
  ...
  try {
    ...
    guideText.textContent = "AI 已就緒，請讓人物完整入鏡";
  } catch (e) {
    console.error("AI model error", e);
    poseLandmarker = null;
    const n=e?.name||"UnknownError", msg=String(e?.message||e||"").replace(/\s+/g," ").slice(0,140);
    setStatus("AI 載入失敗", false);
    guideText.textContent = `${n}${msg ? "：" + msg : ""}`;
