...
async function loadModel() {
  ...
  aiLoading = true;
  if (running) { setStatus("AI 載入中…", true); guideText.textContent = "AI 正在載入…"; }
  try {
    ...
    // iPhone Safari 有些版本對 GPU delegate 不穩，先試 GPU，失敗就自動改 CPU。
    try {
      poseLandmarker = await m.PoseLandmarker.createFromOptions(vision, {
        baseOptions: { modelAssetPath: MODEL_URL, delegate: "GPU" }, ...common
      });
    } catch (gpuError) {
      console.warn("GPU delegate failed, retry CPU", gpuError);
      poseLandmarker = await m.PoseLandmarker.createFromOptions(vision, {
        baseOptions: { modelAssetPath: MODEL_URL, delegate: "CPU" }, ...common
      });
    }
    if (running) {
    ...
    poseLandmarker = null;
    if (running) {
      setStatus("相機已開啟（AI 載入失敗）", true);
      guide.classList.remove("hidden");
      ready.classList.add("hidden");
      arrow.textContent = "↻";
      guideText.textContent = `AI 載入失敗：${e?.name || "錯誤"}`;
    }
...
captureButton.addEventListener("click",()=>{
  ...
  if(facingMode==="user"){p.translate(photo.width,0);p.scale(-1,1);}
  p.drawImage(video,0,0,photo.width,photo.height);
  photo.toBlob(async (blob)=>{
    if(!blob)return;
    const file=new File([blob],`AI攝影師-${Date.now()}.jpg`,{type:"image/jpeg"});
    if(navigator.share && navigator.canShare && navigator.canShare({files:[file]})){
      try{ await navigator.share({files:[file],title:"AI 攝影師照片"}); return; }
...
guide.addEventListener("click",()=>{ if(running && !poseLandmarker) loadModel(); });
